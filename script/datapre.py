...
def str_to_dataset(dataset):
    if dataset == "adult":
        ...
        from module.data_manager.text_classification_manager import IMDB
...
def load_and_partition(seed, dataset, distribution, alpha, num_parts):
    ...
    loader.read(test_ratio=config.test_ratio, shuffle_seed=seed, nrows=config.num_rows_of_dataset[dataset]) # 修改了config 包括了mnist
