...
class ShapleyValue(Measure):
    ...
    def __init__(
        ...
        attacker=None,
        attacker_id=None,
        use_last_round_sv: bool = False,
        last_round_state=None,
        last_round_client_grads: dict | None = None,
        last_round_weights: list | None = None,
        ...
        self.use_last_round_sv = bool(use_last_round_sv)
        self.last_round_state = last_round_state
        self.last_round_client_grads = last_round_client_grads
        self.last_round_weights = last_round_weights
        if self.use_last_round_sv:
            print("[SV v7] use_last_round_sv=True requested, but full-FL final utility is enforced. Ignoring approximation.")
    ...
    def phi(self, i, value_function):
        ...
        for S_size in range(len(all_set) + 1):
            weight = 1.0 / self.num_parts / comb(self.num_parts - 1, S_size)
            for cb in cbs(all_set, S_size):
                set_with_i = set(cb)
                set_with_i.add(i)
                set_without_i = set(cb)
                accu += (
                    self.evaluate_subset_world(set_with_i, value_function)
                    - self.evaluate_subset_world(set_without_i, value_function)
                ) * weight
        return accu
    ...
    def get_contributions(self, **kwargs):
        ...
        for idx_val in range(len(self.value_functions)):
            value_function = self.value_functions[idx_val]
            for i in range(self.num_parts):
                self.contributions[idx_val][i] = self.phi(i, value_function)
